...
# --- IMPORTANTE: Ajuste a importação do SAM para corresponder à sua biblioteca ---
# O código abaixo assume a biblioteca original 'segment-anything'.
...
try:
    ...
    # O bloco 'sam2' foi removido para evitar duplicidade.
     ...
     print("AVISO: Biblioteca 'segment_anything' (ou similar para SAM) não encontrada. SAM será desativado.")
     print("       Verifique o nome correto da biblioteca instalada ('pip list | grep sam') e ajuste a importação.")
except Exception as e:
     print(f"AVISO: Erro ao importar biblioteca SAM: {e}. SAM será desativado.")
...
VIDEO_FPS_TARGET = 3  # Frames por segundo alvo para processamento de IA (não captura) - Mantido como original
CONTEXT_INJECTION_INTERVAL = 4 # Segundos entre injeções de contexto - Mantido como original
...
# ### CORREÇÃO: Mantendo apenas UM bloco de configuração de logging ###
logging.basicConfig(
    level=logging.INFO,
...
# ### CORREÇÃO: Consolidando a inicialização do Gemini ###
genai = None
glm = None
gemini_client_model = None # Variável para guardar o modelo GenerativeModel inicializado
generation_config = None # Definido aqui para ter escopo
try:
    ...
    # 3. Configurações de Geração (para `send_message`) - Opcional, pode usar defaults
...
# ### CORREÇÃO: Substituindo 'catch' por 'except' ###
    ...
    sys.exit(1)
# O bloco 'finally' foi removido pois não era necessário aqui.
# O bloco duplicado de inicialização do Gemini foi removido.
...
def load_models(device: str):
    ...
    # ### CORREÇÃO: Usando variáveis SAM_MODEL_TYPE e SAM_CHECKPOINT_PATH ###
...
class AudioLoop:
    def __init__(self, video_mode=DEFAULT_VIDEO_MODE, user_name=DEFAULT_USER_NAME, device='cpu'):
        ...
        # ### CORREÇÃO: Renomeando fila sam2 para sam ###
    ...
    def _register_active_processors(self):
        ...
        # ### CORREÇÃO: Verificando processador 'sam' ###
    ...
    # ### CORREÇÃO: Renomeando função e referências de sam2 para sam ###
    async def process_sam(self):
            ...
            try:
                ...
                if masks_info:
                     ...
                     if processed_segments:
                         ...
                         }, "SAM Results") # Nome da fila corrigido
                    ...
                    try:
                        ...
                        if dfs_results and isinstance(dfs_results, list):
                            for df in dfs_results:
                                ...
                                if not df.empty:
                                    # Nome da coluna de distância pode variar (ex: 'VGG-Face_cosine')
                                    distance_col_name = f"{model_name}_{distance_metric}"
                                    if distance_col_name not in df.columns:
                                        # Fallback para coluna 'distance' genérica se a específica não existir
                                        if 'distance' in df.columns:
                                            distance_col_name = 'distance'
                                        else:
                                            logger.warning(f"Coluna de distância '{distance_col_name}' ou 'distance' não encontrada no DataFrame DeepFace. Pulando rosto.")
                                            continue # Pula este DataFrame se não encontrar a coluna
                                    ...
                                    if 'source_x' in df.columns:
                                        ...
                                        logger.warning("Não foi possível encontrar colunas de BBox ('source_x/y/w/h' ou 'region') no resultado DeepFace.")
    ...
    async def context_synthesizer(self):
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                wait_time = max(0.1, 1.0 - synth_elapsed) # Mantido como no original (aprox 1s)
                 ...
                 if text_input.lower() == "q":
                     ...
                     break # Sai do loop da send_text_command
                      ...
                      if len(parts) == 2 and parts[1]:
                           ...
                           # ### CORREÇÃO: Usando model_name correto na mensagem de log ###
    ...
    async def gemini_communication_manager(self):
        ...
        """
        # ### CORREÇÃO: Usando a variável global 'gemini_client_model' ###
            ...
            try:
                ...
                if content_parts and chat:
                    ...
                    try:
                        ...
                        # ### CORREÇÃO: Usando a variável global 'generation_config' ###
        ...
        try:
            ...
            async for chunk in response_stream:
                ...
                try:
                    ...
                    # Pode dar erro se o chunk não tiver candidates ou parts (ex: erro na API)
                    if hasattr(chunk, 'prompt_feedback') and chunk.prompt_feedback.block_reason:
                        logger.warning(f"Prompt bloqueado durante stream: {chunk.prompt_feedback.block_reason}")
                    else:
        ...
        # Captura grupos que contêm QUALQUER outra exceção (incluindo misturas com CancelledError)
            ...
            if other_errors:  # Se houve erros além de cancelamentos
                ...
                for i, exc in enumerate(other_errors.exceptions):
                    ...
                    # Tentar mapear a exceção para a task (pode ser impreciso)
                    # Esta lógica é complexa e omitida para manter a simplicidade pedida
...
async def main():
    ...
    global YOLO_MODEL_PATH, SAM_MODEL_TYPE, SAM_CHECKPOINT_PATH, MIDAS_MODEL_NAME, KNOWN_FACES_DIR, VISION_API_CREDENTIALS, GEMINI_MODEL_NAME, gemini_client_model, API_KEY, SYSTEM_INSTRUCTION
    ...
    # ### CORREÇÃO: Usando nomes de args consistentes com variáveis globais SAM ###
    ...
    # ### CORREÇÃO: Usando nome de arg consistente com variável global Gemini ###
    ...
    # A variável gemini_client_model já foi declarada global no início da função
    try:
        # Verifica se o modelo solicitado é diferente do já inicializado
        if not gemini_client_model or gemini_client_model.model_name != GEMINI_MODEL_NAME:
            logger.info(f"Reconfigurando cliente Gemini para usar modelo: {GEMINI_MODEL_NAME}")
            # A API Key já deve estar configurada globalmente, mas reconfigurar é seguro
            genai.configure(api_key=API_KEY)
            gemini_client_model = genai.GenerativeModel(
                model_name=GEMINI_MODEL_NAME,
                system_instruction=SYSTEM_INSTRUCTION
            ...
            logger.info(f"Modelo Generativo Gemini '{GEMINI_MODEL_NAME}' já está configurado.")
    # ### CORREÇÃO: Usando 'except' em vez de 'catch' ###
...
if __name__ == "__main__":
    ...
    try:
        ...
        if 'pya' in globals() and pya: # Verifica se pya foi inicializado
            try: pya.terminate()
