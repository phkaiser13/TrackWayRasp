...
DANGER_CLASSES = [
    ...
    'knife', 'scissors', 'razor', 'saw', 'axe', 'hatchet',
    # armas de fogo
    'gun', 'pistol', 'rifle', 'shotgun', 'revolver',
    # explosivos e inflamáveis
    'bomb', 'grenade', 'fire', 'flame', 'smoke', 'lighter', 'matches',
    # superfícies e situações de calor
    'stove', 'hot surface', 'candle', 'burner',
    # eletricidade e cabos
    'live_wire', 'electric_outlet', 'battery',
    # vidros e pontas
    'broken_glass', 'shard', 'needle', 'syringe',
    # ferramentas pesadas
    'hammer', 'wrench', 'drill', 'chainsaw',
    # veículos em movimento
    'car', 'motorcycle', 'bicycle', 'truck', 'bus',
    # animais potencialmente perigosos
    'bear', 'snake', 'spider', 'alligator',
    ...
    types.Tool(
        function_declarations=[
            types.FunctionDeclaration(
                ...
                description="Use esta função para salvar o rosto da pessoa atualmente em foco pela câmera. A função requer o nome da pessoa como parâmetro para identificar e armazenar o rosto no banco de dados de rostos conhecidos (localizado em DB_PATH/known_faces). Se o nome da pessoa não for fornecido pelo usuário na solicitação inicial, você (Trackie) DEVE primeiro perguntar ao usuário qual nome ele deseja usar, e SÓ ENTÃO chamar esta função com o nome fornecido.",
                parameters=types.Schema(
                    type=types.Type.OBJECT,
                    properties={
                        "person_name": types.Schema(type=types.Type.STRING, description="O nome da pessoa a ser salva. Este nome deve ser obtido do usuário ANTES de chamar esta função, caso não tenha sido fornecido na primeira interação.")
                    },
...
*   **`save_known_face(person_name: string)`**:
    *   Use esta função para salvar o rosto da pessoa atualmente em foco. O parâmetro `person_name` é obrigatório.
    *   **Se o usuário fornecer o nome diretamente** (ex: "Salve este rosto como Pedro"):
        1.  Você (Trackie) DEVE responder primeiro COM VOZ: "Usuário, salvando rosto de [NOME], um momento..."
        2.  Então, chame esta função `save_known_face` com o `person_name` fornecido.
    *   **Se o usuário NÃO fornecer o nome** (ex: "Salve este rosto"):
        1.  Você (Trackie) DEVE perguntar COM VOZ: "Qual nome você gostaria de usar para salvar este rosto?"
        2.  Aguarde a resposta do usuário com o nome.
        3.  Após receber o nome, você (Trackie) DEVE responder COM VOZ: "Usuário, salvando rosto de [NOME OBTIDO], um momento..."
        4.  Então, chame esta função `save_known_face` com o `person_name` obtido.
    *   A função Python `_handle_save_known_face` fará o salvamento real e retornará uma mensagem de sucesso ou falha. Você deve relatar este resultado final ao usuário COM VOZ.
    **SUPER IMPORTANTE**: O feedback "Salvando rosto de [NOME]..." e o resultado final (ex: "Rosto de Pedro salvo.") devem ser falados imediatamente, sem esperar novo input do usuário.

*   **`identify_person_in_front()`**: Chamada quando o usuário pergunta "quem está na minha frente?" ou similar.
    *   ANTES de chamar a função Python, você (Trackie) DEVE responder COM VOZ: "Usuário, identificando pessoa, um momento..."
    *   Após a função Python retornar, você informará o resultado final ao usuário COM VOZ, imediatamente.
...
*   **`find_object_and_estimate_distance(object_description: string, object_type: string)`**:
    ...
    *   ANTES de chamar a função Python, você (Trackie) DEVE responder COM VOZ: "Usuário, localizando [DESCRIÇÃO DO OBJETO], um momento..."
    *   Sua resposta final deve usar o resultado da função e ser dita COM VOZ, imediatamente. Ex: "Usuário, o computador azul está a aproximadamente 5 passos, ligeiramente à direita." ou "Usuário, a chave de fenda preta está sobre a mesa, a cerca de 2 passos à sua frente.". Se a função retornar que não encontrou: "Usuário, não consegui localizar o objeto [descrição]."
...
=== 6. Exemplo de Interação (Salvar Rosto - Nome Fornecido) ===
*   Usuário (Voz): "Trackie, salve este rosto como Ana."
*   Trackie (Voz, ANTES da função): "Usuário, salvando rosto de Ana, um momento..."
*   (Sistema chama `save_known_face(person_name="Ana")`)
*   (Função retorna: `{"status": "success", "message": "Rosto de Ana salvo com sucesso."}`)
*   Trackie (Voz, FINAL): "Usuário, rosto de Ana salvo com sucesso."

=== 7. Exemplo de Interação (Salvar Rosto - Nome NÃO Fornecido) ===
*   Usuário (Voz): "Trackie, salve este rosto."
*   Trackie (Voz): "Qual nome você gostaria de usar para salvar este rosto?"
*   Usuário (Voz): "Carlos."
*   Trackie (Voz, ANTES da função): "Usuário, salvando rosto de Carlos, um momento..."
*   (Sistema chama `save_known_face(person_name="Carlos")`)
*   (Função retorna: `{"status": "success", "message": "Rosto de Carlos salvo com sucesso."}`)
*   Trackie (Voz, FINAL): "Usuário, rosto de Carlos salvo com sucesso."
        """
)],
        role="user"
    ),
...
class AudioLoop:
    ...
    def __init__(self, video_mode: str = DEFAULT_MODE, show_preview: bool = False):
        ...
        try:
            ...
            midas_transforms = torch.hub.load("intel-isl/MiDaS", "transforms") # Corrigido para carregar 'transforms' corretamente
            if MIDAS_MODEL_TYPE == "MiDaS_small":
                 self.midas_transform = midas_transforms.small_transform
            elif MIDAS_MODEL_TYPE == "dpt_swin2_large_384": # Exemplo de outro modelo
                 self.midas_transform = midas_transforms.dpt_transform # Ajustar se o nome do transform for diferente
            else: # Fallback para um transformador DPT genérico se não for small
            ...
            while not self.stop_event.is_set():
                ...
                if yolo_alerts and self.session:
                    for alert_class_name in yolo_alerts:
                        try:
                            alert_msg = f"YOLO_ALERT: {alert_class_name.upper()} detectado!" # Gemini irá formatar para "Usuário, CUIDADO!..."
                            print(f"Enviando para Gemini (para verbalização): {alert_msg}")

                            # opcional: limpa espaço na fila se estiver cheia
                            if self.out_queue and self.out_queue.full():
                                await self.out_queue.get()
                                print("Aviso: Fila de saída cheia, descartando alerta antigo para enviar novo.")

                            # envia diretamente pro Gemini
                            await self.session.send(input=alert_msg, end_of_turn=True) # end_of_turn=True para processamento imediato
                        except Exception as e:
            ...
            while not self.stop_event.is_set():
                ...
                try:
                    if isinstance(msg, dict):
                        ...
                        print(f"Enviando alerta/texto para Gemini processar: {msg}")
                        # Alertas ou texto informativo são enviados para Gemini interpretar e falar
                        # end_of_turn=True para que Gemini processe e responda imediatamente (ex: alertas YOLO)
                        await self.session.send(input=msg, end_of_turn=True if "YOLO_ALERT" in msg else False)
                    self.out_queue.task_done()
    ...
    def _handle_identify_person_in_front(self) -> str:
        ...
        try:
            ...
            elif 'distance' in df.columns: distance_col_name = 'distance' # Fallback
            else: # Tenta encontrar dinamicamente
            ...
            # Limiares de exemplo, podem precisar de ajuste fino
            threshold = 0.6 # Default para VGG-Face com cosine
            ...
            # Adicione outros modelos/métricas conforme necessário

            if distance > threshold:
                print(f"[DeepFace] Distância {distance:.4f} > limiar ({threshold}). Não reconhecido com alta confiança.")
                # Poderia retornar algo como "Vejo um rosto, mas não tenho certeza de quem é."
                return "Não tenho certeza quem é, mas detectei um rosto."


            return f"A pessoa na sua frente parece ser {person_name}."

        except ValueError as ve: # Geralmente quando nenhum rosto é detectado
            print(f"[DeepFace] Erro (ValueError) ao identificar: {ve}")
            return "Não detectei um rosto claro para identificar, ou houve um problema ao buscar no banco de dados."
        except Exception as e:
    ...
    def _handle_find_object_and_estimate_distance(self, object_description: str, object_type: str) -> str:
        ...
        if depth_map is not None:
            try:
                ...
                if depth_value > 1e-6:
                     # Esta escala é altamente dependente do modelo MiDaS e da cena.
                     # Para MiDaS_small, a saída é profundidade relativa (não métrica).
                     # A calibração seria necessária para precisão.
                     # Vamos usar uma heurística muito simples e deixar claro que é uma estimativa.
                     # Exemplo: assumir que um valor de profundidade menor significa mais perto.
                     # E mapear o intervalo de `depth_value` para um intervalo de metros.
                     # Isso é muito rudimentar.
                     if MIDAS_MODEL_TYPE == "MiDaS_small":
                        # Heurística para MiDaS_small (profundidade relativa)
                        # Quanto maior o depth_value, mais longe (inverso da profundidade real)
                        # Ajuste este fator '50' com base em testes.
                        estimated_meters = 50 / (depth_value + 1e-6) # Adiciona epsilon para evitar divisão por zero
                     else: # Para modelos DPT que podem dar saída mais próxima de métrica (ainda pode precisar de escala)
                        estimated_meters = depth_value # Ou depth_value * fator_escala

                     estimated_meters = max(0.3, min(estimated_meters, 20)) # Limita a um range razoável
                     ...
                     print("[Localizar Objeto] Valor de profundidade MiDaS inválido ou muito pequeno no centro do objeto.")
        ...
        object_name_to_report = object_description # Usar a descrição original do usuário na resposta

        response_parts = [f"o {object_name_to_report}"]
        if is_on_surface:
        ...
        if len(response_parts) == 1: # Só tem o nome do objeto, algo deu errado
            return f"Usuário, encontrei o {object_name_to_report} {direction}, mas não consegui estimar a distância."
        
        return f"Usuário, {' '.join(response_parts)}."
    ...
    async def receive_audio(self):
        ...
        try:
                ...
                try:
                    ...
                    async for response_part in self.session.receive():
                        ...
                        if getattr(response_part, "function_call", None):
                            ...
                            # Definir o evento de "pensando" aqui, antes de qualquer processamento da função.
                            # Gemini já deve ter dado o feedback verbal inicial (ex: "Salvando rosto de...")
                            # conforme as system_instructions.
                            ...
                            try:
                                if self.video_mode != "camera" and (function_name in ["save_known_face", "identify_person_in_front", "find_object_and_estimate_distance"]):
                                    ...
                                    if function_name == "save_known_face":
                                        ...
                                        if person_name_arg:
                                            ...
                                            # Este caso indica que Gemini não seguiu a instrução de 'required'
                                            # ou a lógica de pedir o nome primeiro.
                                            result_message = "ERRO_INTERNO_GEMINI: A função 'save_known_face' foi chamada sem 'person_name'. O nome é obrigatório e deve ser solicitado ao usuário primeiro."
                                            print(f"  [Trackie] Erro: {result_message}")
                                ...
                                # Limpar o evento de "pensando" após a função ter sido processada e a resposta enviada.
                    ...
                    if not self.stop_event.is_set():
                        if has_received_data_in_turn:
                            ...
                            # Não necessariamente um erro, pode ser apenas o fim de um stream de áudio sem texto ou função.
                            # print("Stream do turno atual terminou sem dados novos visíveis (texto/função).")
            ...
            self.thinking_event.clear() # Garantir que seja limpo ao sair
    ...
    async def play_audio(self):
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                try:
                    ...
                    if bytestream is None:
                        ...
                        # self.stop_event.set() # Não necessariamente parar tudo, apenas esta tarefa pode terminar se a fila for fechada com None
                        break # Sai do loop de play_audio

                    if stream and stream.is_active():
                        await asyncio.to_thread(stream.write, bytestream)
                    else:
                        print("Stream de áudio para playback não está ativo. Descartando áudio.")
                        # Se o stream não estiver ativo, pode ser necessário recriá-lo ou parar.
                        # Por ora, apenas logamos e descartamos.
            ...
            traceback.print_exc()
            # self.stop_event.set() # Considerar se este erro deve parar tudo
        finally:
    ...
    async def run(self):
        ...
        while attempt <= max_retries and not self.stop_event.is_set():
            try:
                if attempt > 0:
                     ...
                     retry_delay = min(retry_delay * 2, 30) # Backoff exponencial com teto
                ...
                async with client.aio.live.connect(model=MODEL, config=CONFIG) as session:
                    ...
                    # self.out_queue já é inicializado no __init__, mas podemos recriar se necessário
                    # para garantir que esteja limpo para a nova sessão.
                    self.out_queue = asyncio.Queue(maxsize=100)
                    self.stop_event.clear() # Limpa o evento de parada para a nova sessão
                    self.thinking_event.clear() # Limpa o evento de pensamento
                    ...
                    if not self.stop_event.is_set():
                         print("Sessão Gemini terminou ou TaskGroup concluído inesperadamente. Tentando reconectar...")
                         attempt += 1
                         self.session = None

            except asyncio.CancelledError:
                print("Loop principal (run) cancelado.")
                self.stop_event.set() # Garante que o loop while pare
                break
                ...
                for i, exc in enumerate(eg.exceptions):
                    ...
                    traceback.print_exc() # Imprime traceback para cada exceção no grupo
                attempt += 1
                self.session = None
            except Exception as e:
                ...
                attempt += 1
                self.session = None
                if "LiveSession" in str(e) or "DeadlineExceeded" in str(e):
                     print("Erro relacionado à sessão Gemini. Tentando reconectar.")
        ...
        if self.session:
            print("Sessão LiveConnect gerenciada pelo context manager será fechada (ou já foi).")
            self.session = None

        if self.audio_in_queue and self.audio_in_queue.empty():
            try: self.audio_in_queue.put_nowait(None) # Sinaliza para play_audio terminar
            except asyncio.QueueFull: pass
        elif self.audio_in_queue: # Se não estiver vazia, tenta limpar
            while not self.audio_in_queue.empty():
                try: await self.audio_in_queue.get()
                except: break
...
if __name__ == "__main__":
    ...
    if args.mode == "camera":
        ...
        if not os.path.exists(DB_PATH):
            try:
                ...
                # exit(1) # Considerar se deve sair ou continuar sem DB
    ...
    try:
        ...
        if not hasattr(main_loop, 'run'): # Verificação de segurança
    ...
    except AttributeError as ae: # Captura erros de atributo mais especificamente
        ...
        traceback.print_exc()
        if main_loop: main_loop.stop_event.set() # Tenta parar o loop em caso de erro
    finally:
        print("Bloco __main__ finalizado.")
        # Pequena pausa para garantir que as mensagens de finalização das tarefas sejam impressas
        # await asyncio.sleep(1) # Não pode usar await aqui, pois não está em um contexto async
